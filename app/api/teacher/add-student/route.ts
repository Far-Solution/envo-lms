// app/api/admin/add-student/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const { firstName, lastName, phone, courseId } = body
    ...
    // Step 5: Link student to selected course
    const { error: studentCourseError } = await supabaseAdmin.from("student_courses").insert({   // ✅ changed
      course_id: courseId,
      profile_id: userData.user.id,
    })

    if (studentCourseError) {
      return NextResponse.json({ error: studentCourseError.message }, { status: 500 })
    }
